...
function createApp() {
  ...
  * */
  ...
  app.prototype.getId = function GetApplicationID() {
    ...
    var appId = readAppId('/app/.app_id');
    if(!appId) {
      appId = readAppId('./app_id');
    }
    if(!appId) { // generate.
      appId = this.name() + '-' + util.randomString(10);
    }
    if(!writeAppId(appId, '/app/.app_id') && !writeAppId(appId, '/.app_id')) {
      console.warn('Crux.app: failed to persist app id.');
    }
    return appId;
  };

  function writeAppId(appId, path) {
    try {
      fs.writeFileSync(__rootdir + path, appId, { encoding: 'utf8' });
      return true;
    } catch(e) {
      return false;
    }
  ...
  function readAppId(path) {
    try {
      var appId = fs.readFileSync(__rootdir  + path, { encoding: 'utf8' });
      if(typeof appId !== 'string' || !appId) throw new Error('NO_APP_ID');
      return appId;
    } catch(e) {
      return false;
    }
