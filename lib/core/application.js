...
function createApp() {
  ...
  /* Places the crux framework to the global scope, so that you don't have to require('crux')every time */

  app.prototype.globalize = function GlobalizeCrux() {
    global['crux'] = require('../../index');
    return this;
  };
  ...
  * */
  ...
  app.prototype.addComponent = function AddComponent(component, _config) {
    ...
    if(_.isCruxComponent(component)) {
      ...
      if(component.name.indexOf('.') !== -1) {  // We try and see if the name is like "redis.something", to see if we have the config.redis.something definition.
        // TODO
      }
      return this;
    }
