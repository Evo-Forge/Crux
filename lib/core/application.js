...
function createApp() {
  ...
  /**
   * Working with environment variables can be a pain for many node.js projects. CRUX makes your life easier.
   * Just place the "ENV_FILE_PATH" variable in environment variables, with the project-relative path of
   * a file containing KEY=VALUE\n environment variable pairs. CRUX will then load the file content,
   *  place it under process.env[key] = value;
   * */
  (function loadEnvironmentFile() {
    if(typeof process.env.ENV_FILE_PATH === 'undefined') return;  //nothing to load.
    var envPath = path.normalize(__rootdir + '/' + process.env.ENV_FILE_PATH);
    try {
      var envContent = fs.readFileSync(envPath, { encoding: 'UTF8' });
    } catch(e) {
      console.error("Crux.app: failed to read environment file path: %s", envPath);
      return;
    }
    ...
    for(var i=0; i < lines.length; i++) {
      ...
      var tmp = lines[i].split('='),
        key = tmp[0],
        value = tmp[1];
      // Some custom variables are skipped, such as NODE_ENV
  ...
  var appPackage = null,// information found in package.json
    configEnv = null, // we use this path to dynamically load configuration files based on the environment
    registryObj,
  ...
  app.prototype.name = function AppName(_value) {
    ...
    return (appName ? appName : appPackage && appPackage.name);
  };

  /*
   * Returns the app version if set
   * */
  app.prototype.version = function GetVersion() {
    if(appPackage != null) {
  ...
  /*
   * Loads information in the package.json file, and attaches it to crux.app
   * */
  function loadInfo() {
    try {
  ...
  /*
   * Reads the application's extensions.
   * */
  var wasLoaded = false;
  function extendDir(_dir) {
    var files;
    try {
      files = util.readDirectory(path.normalize(__rootdir + _dir), 'js');
    } catch (e) {
      // We have no extendings.
      console.warn('Crux.app: extend folder "%s" is not available.', _dir);
    }
  ...
  app.prototype.loadExtensions = function LoadExtensions() {
    if(wasLoaded) return this;
    wasLoaded = true;
    if (projectConfig.crux.extends === null) return this;
    if(typeof projectConfig.crux.extends === 'string') {
      extendDir(projectConfig.crux.extends);
      return this;
    }
    if(projectConfig.crux.extends instanceof Array) {
      for(var i=0; i < projectConfig.crux.extends.length; i++) {
        extendDir(projectConfig.crux.extends[i]);
      }
    }
    return this;
  };
  ...
  app.prototype.init = function InitializeApplication() {
    ...
    this.environment();
    // We then gather information about the application, by reading the package.json file, if available.
    loadInfo.call(this);
  ...
  /**
   * This will generate a unique app ID, persistent to node restarts. By default, it will generate a .app_id file under __rootdir/app/.app_id
   * OR under __rootdir/.app_id
   * NOTE: the following format will be used:
   * {appName}-{random10String}
   * */
  var __APP_ID = null;
  app.prototype.getId = function GetApplicationID() {
    if(__APP_ID) return __APP_ID;
    var appName = this.name();
    var appId = readAppId('/app/.app_id', appName);
    if(!appId) {
      appId = readAppId('./app_id', appName);
    }
    if(!appId) { // generate.
      appId = appName + '-' + util.randomString(10);
    }
    if(writeAppId('/app/.app_id', appName, appId)) {
      return appId;
    }
    if(writeAppId('/.app_id', appName, appId)) {
      return appId;
    }
    console.warn('Crux.app: failed to persist app id.');
    return appId;
  };

  function writeAppId(path, appName, appId) {
    var content = {};
    try {
      var tmp = fs.readFileSync(__rootdir + path, { encoding: 'utf8' });
      tmp = JSON.parse(tmp);
      if(typeof tmp !== 'object' || !tmp) throw 1;
      content = tmp;
    } catch(e) {}
    content[appName] = appId;
    try {
      fs.writeFileSync(__rootdir + path, JSON.stringify(content), { encoding: 'utf8' });
      return true;
    } catch(e) {
      return false;
    }
  ...
  function readAppId(path, appName) {
    try {
      var tmp = fs.readFileSync(__rootdir  + path, { encoding: 'utf8' });
      if(typeof tmp !== 'string' || !tmp) throw 1;
      tmp = JSON.parse(tmp);
      return tmp[appName] || false;
    } catch(e) {
      return false;
    }
  }
