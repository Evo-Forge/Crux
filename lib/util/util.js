var crypto = require('crypto'),
  ...
  fs = require('fs');
/*
* Krux general utilities found under krux.util
* */
...
util.promise = function CreatePromise(handler) {
  var pObj = new bluebird(function(resolve, reject) {
    try {
      ...
      log.error('Krux.promise: Encountered an error.');
      log.debug(e);
      reject(e);
    }
  });
...
util.readDirectory = function ReadDirectory(path, extension, str, isSingleLevel) {
  ...
  if(typeof path !== 'string' || !path) {
    throw new Error('Krux.util.readDirectory: path is not a string');
  }
...
util.getFileName = function GetFileName(_full, withExtension) {
  if(typeof _full !== 'string') throw new Error("Krux.util.getFileName: path not a string");
