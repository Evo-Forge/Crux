...
master.prototype.bind = function BindEvents() {
  ...
  this.io.on('connection', function(socket) {
    function onAuth(data, fn) {
      self.auth(socket, data, fn);
    }
    function onRegister(options) {
    ...
    function onDisconnect() {
      socket.removeListener('auth', onAuth);
      socket.removeListener('register', onRegister);
...
master.prototype.register = function RegisterNode(socket, data) {
  ...
  function onStats(data) {
    self.stats.call(self, socket, data);
  }
  function onDisconnect() {
    self.disconnect(socket);
    socket.removeListener('disconnect', onDisconnect);
    socket.removeListener('status', onStats);
  }
...
/*
* Called when we receive a stats information from a client.
* */
master.prototype.stats = function OnStats(socket, stats) {
  var id = socket._NODE_ID;
  for(var i=0; i < this.connected.length; i++) {
    if(this.connected[i].id === id) {
      this.connected[i].stats = stats;
      break;
    }
  }
};
...
master.prototype.disconnect = function OnSocketDisconnect(socket) {
  ...
  // We remove all socket listeners.
  for(var e in socket._events) {
