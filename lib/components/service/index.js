...
/*
* This is the Krux Service component. It allows service registration and access
...
Service.prototype.init = function InitializeServices() {
  ...
  this.registry().attachTo('service server', 'service', getService);
};
...
function loadServices() {
  ...
  try {
    var list = Krux.util.readDirectory(serviceDir, 'js', null, true);
  } catch(e) {
    // No services.
    log.warn('Krux.service: No services found in: %s', this.config.path);
    return;
  }
  var toInit = [],
    valid = 0,
  ...
  list.forEach(function(servicePath) {
    ...
    if(!_.isKruxService(serviceObj)) {
      var fileName = servicePath.replace(serviceDir, '');
      fileName = fileName.substr(1, fileName.length-1);
      log.warn('Krux.service: %s does not extend Krux.Service. Skipping', fileName);
      return;
    }
    if(_.isFunction(serviceObj.init)) {
      toInit.push(serviceObj);
    }
  ...
  if(valid === 0) {
    log.warn('Krux.service: No valid services found in: %s', this.config.path);
  }
};
