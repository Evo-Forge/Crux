 ...
 *   </pre>
 * @class crux.Service
 * @extends crux.Component, EventEmitter
 * @interface
 * @example
...
Service.prototype.init = function InitializeServices() {
  ...
  this.registry().attachTo('service server io tasks', 'service', getService);
...
function loadServices() {
  ...
  _.forEach(paths, function(sPath) {
    ...
    try {
      ...
      log.warn('Crux.service: No services found in: %s', sPath);
      return;
    }
    list.forEach(function(servicePath) {
      ...
      _.forEach(EventEmitter.prototype, function(val, key) {
        serviceObj.__proto__[key] = val;
      });
      /* Add support for multiple events listening */
      var _on = serviceObj.__proto__.on;
      serviceObj.__proto__.on = function OnEventWrapper(events, callback) {
        if(!(events instanceof Array)) return _on.apply(this, arguments);
        for(var i=0; i < events.length; i++) {
          _on.call(this, events[i], callback);
        }
        return this;
      };
      /* Add support for multiple event unsubscribe. */
      var _off = serviceObj.__proto__.off;
      serviceObj.__proto__.off = function OffEventWrapper(events, callback) {
        if(!(events instanceof Array)) {
          return _off.apply(this, arguments);
        }
        for(var i=0; i < events.length; i++) {
          _off.call(this, events[i], callback);
        }
        return this;
      };
      EventEmitter.call(serviceObj);
      serviceObj.name = serviceName;
      serviceObj.init(serviceConfig);
    });
  });
  if(valid === 0) {
    log.warn('Crux.service: No valid services found in: %s', self.config.path);
  }
}
