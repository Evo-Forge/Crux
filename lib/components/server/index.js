...
Component.default({
  ...
  cookie: { // If we use cookies, these are the connect cookie settings
    ...
    secure: false,
    store: {
      type: 'memory'  // The default session store. Current values are: "memory",  "file" or "custom"
    }
  },
  request: {
    ...
    cors: false,          // Should we allow CORS requests. By default, set to false, can also be set to the allowed methods, ex: GET, POST, OPTIONS
    cors_domain: false,   // when set, we will only set this domain.
    cors_static: false,   // Should we allow CORS requests for static files only, eg: .js|css|imgs
    ...
    origins_static: true,  // Enable or disable the static assets for origins.
  ...
  path: {                       // Paths relative to the app's rootdir
    ...
    public: 'public/',          // Public folder location. Set to "false" if we do not serve public assets
...
const DEFAULT_STATIC_EXTENSIONS = ['js','css','png','jpg','jpeg','gif','woff','woff2', 'svg','eot','ttf','swf'];
 ...
 * @param {Boolean} [config.request.cors=false] - enables or disables Cross-Origin Requests, by adding or removing Access-Controll headers. Can be a string containing the actual whitelisted methods
...
Server.prototype.packages = function GetPackages() {
  var list = ['express@3.21.2', 'body-parser@1.13.x', 'request@2.60.x', 'method-override@2.3.x'];
  if(this.config.path.public) {
    list.push('serve-static@1.10.x');
  }
  ...
  if(this.config.request.cookies) {
    ...
    if(this.config.cookie.store && this.config.cookie.store.type) {
      if(this.config.cookie.store.type === 'file') { // file sessions.
        list.push('session-file-store@0.0.22');
        if(!this.config.cookie.store.path || this.config.cookie.store.path.charAt(0) === '.') {
          this.config.cookie.store.path = path.normalize(__rootdir + '/' + this.config.cookie.store.path || '');
          this.config.cookie.store.logFn = function(){};
        }
      }
    }
  }
  if(typeof this.config.views === 'object') {
...
Server.prototype.init = function InitializeServer() {
   ...
   * */
  if(this.config.cookie && this.config.cookie.store) {
    if(this.config.cookie.store.type === 'memory') {
      Component.lib('sessionStore', express.session.MemoryStore, true);
    } else if(this.config.cookie.store.type === 'file') {
      var sessFileStore = require('session-file-store');
      Component.lib('sessionStore', sessFileStore(express.session));
    }
  }
...
Server.prototype.run = function RunServer(cb) {
  if(this.config.request.cors_static === true) {
    this.config.request.cors_static = DEFAULT_STATIC_EXTENSIONS;
  }
  if(typeof this.config.views === 'object') {
...
/**
* Close any remaining connections and gracefully stop the express HTTP server.
 * @memberof crux.Server.Server
 * @function stop
 * @instance
 * @override
 * @param {Function} fn - the callback function to call on stopped.
* */
Server.prototype.stop = function CloseServer(fn) {
  var self = this;
  this.http.close(function(e) {
    if(self.config.debug) {
      log.trace('crux.Server: Server closed and connections draining complete.');
    }
    fn && fn(e);
  });
  return this;
...
/**
 * CORS requests for static files.
 * @memberof crux.Server.Server
 * @function staticCorsRequest
 * @instance
 * */
Server.prototype.staticCorsRequest = function HandleStaticCorsRequest() {
  var STATIC_EXTENSIONS = this.config.request.cors_static;
  return function StaticCORSRequest(req, res, next) {
    if(req.method !== 'OPTIONS' && req.method !== 'GET') {
      return next();
    }
    var idx = req.path.lastIndexOf('.');
    if(idx === -1) return next();
    var ext = req.path.substr(idx+1).toLowerCase();
    if(ext.indexOf('?') !== -1) {
      ext = ext.split('?')[0];
    }
    if(STATIC_EXTENSIONS.indexOf(ext) === -1) {
      return next();
    }
...
Server.prototype.corsRequest = function HandleCorsRequest() {
  ...
  var originDomain = this.config.request.cors_domain || false;
  return function CORSRequest(req, res, next) {
    var headers = (typeof req.headers === 'object' && req.headers ? req.headers : {});
    res.header('Access-Control-Allow-Origin', originDomain || headers['origin'] || '*');
...
Server.prototype.originVerification = function HandleOriginRequest() {
  var domains = this.config.request.origins,
    allowAssets = this.config.request.origins_static;
  ...
  return function OriginVerificationRequest(req, res, next) {
    ...
    // IF we have a static file, we check the origins_static setting
    if(allowAssets && req.method === 'GET') {
      var _tmpUrl = req.url,
        qsIdx = _tmpUrl.indexOf('?');
      if(qsIdx !== -1) {
        _tmpUrl = _tmpUrl.substr(0, qsIdx);
      }
      var fileName = _tmpUrl.split('/').pop(),
        extIdx = fileName.lastIndexOf('.');
      if(extIdx !== -1) {
        var _ext = fileName.substr(extIdx+1);
        if(DEFAULT_STATIC_EXTENSIONS.indexOf(_ext) !== -1) {
          return next();
        }
      }
      return next();
...
Server.prototype.requestStart = function HttpRequestStart(endpoint, reqObj) {
  if(!reqObj || !reqObj.endpoint || !reqObj.endpoint.__trace) return;
 ...
 * */
Server.prototype.requestEnd = function HttpRequestEnd(reqObj, data) {
  ...
  var took = (new Date().getTime() - reqObj.__start_time),
    _path = reqObj.req ? reqObj.req.path : reqObj.path,
    _msg = '';
  try {
    if(data.type === 'error') {
      _msg += ' [' + data.code + ']';
  ...
  log.trace("%s %s (%s:%s) " + took + "ms" + _msg, reqObj.__method.toUpperCase(), _path, reqObj.namespace, reqObj.name);
  try {
    reqObj.destroy();
  } catch(e){}
};
...
Server.prototype.configureDefaults = function Configure() {
  ...
  // Static CORS checking
  if(this.config.request.cors_static instanceof Array) {
    this.app.use(this.staticCorsRequest());
  }
  // Origin check
  ...
  this.app.use(function(req, res, next) {
    ...
    function onEnd() {
      ...
      if(contentType.indexOf('json') !== -1 || contentType.indexOf('form') !== -1) {
        var json;
        try {
          if(typeof req.rawBody === 'undefined' || req.rawBody === '') {
            json = {};
          } else {
    ...
    if(typeof req.body === 'object' && req.body && Object.keys(req.body).length > 0) {
      return next();
    }
    req
  ...
  if(this.config.request.cookies) {
    ...
    if(typeof SessionStore.instance === 'function') {
      ...
      if(typeof SessionStore === 'object') {
        ...
        if(this.config.cookie.store.type === 'file') {
          delete this.config.cookie.store.type;
          sessionStoreObj = new SessionStore(this.config.cookie.store);
        } else {
    ...
    Component.lib('store', sessionStoreObj, true);
...
Server.prototype.bind = function Bind(callback) {
  ...
  crux.app.emit('server:listen', this.http);
  this.http.listen(this.config.port, this.config.ip, function() {
    if(cbCalled) return;
    log.info('Crux.server started on %s:%s', self.config.ip, self.config.port);
    cbCalled = true;
    callback();
  });
};
...
Server.prototype.pageNotFound = function PageNotFound(req, res) {
  if(!res._headerSent) res.status(404);
...
Server.prototype.serverError = function ServerError(err, req, res, next) {
  if(err instanceof SyntaxError && typeof err.body !== 'undefined') {
    if(!res._headerSent) res.status(500);
    return res.json({
      type: 'error',
      code: 'INVALID_JSON'
    });
  ...
  if(!err.custom) {
    if(err.type === 'entity.too.large') {
      err.message = 'Entity too large.';
    } else {
      log.warn('CruxServer Encountered an error in [%s %s]' , req.method, req.path);
      log.debug(err);
      if(!this.config.request.restful) {
        err.message += " (Id: " + errId + ")";
      }
      try {
        if(!err.custom) {
          err.message = err.message.split(path.normalize(global["__rootdir"])).join("");
        }
      } catch(e) {}
    }
  } else {
  ...
  if(isAjax) {
    return res.json({
      type: 'error',
      code: err.custom ? err.code || 'SERVER_ERROR' : 'INTERNAL_ERROR',
      message: err.custom ? err.message || 'An unexpected error occurred.' : 'An internal error occurred'
    });
  }
