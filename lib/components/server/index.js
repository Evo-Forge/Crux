...
Server.prototype.configureDefaults = function Configure() {
  ...
  if(this.config.request.cookies) {
    ...
    var SessionStore = Component.lib('sessionStore'),
      sessionStoreObj;
    if(typeof SessionStore.__proto__ === 'undefined') {
      sessionStoreObj = new SessionStore(express.session.Store, express.session.Session);
    } else if(typeof SessionStore.instance === 'function') {
      sessionStoreObj = SessionStore.instance(express.session.Store, express.session.Session);
    } else {
...
Server.prototype.pageNotFound = function PageNotFound(req, res) {
  res.status(404);
  var isAjax = req.accepts('json') && (!req.accepts('html') || req.xhr || this.config.request.restful);
  if(isAjax) {
    return res.json({
      type: 'error',
      code: 'NOT_FOUND',
...
Server.prototype.serverError = function ServerError(err, req, res) {
  ...
  if(!req.accepts('html') || req.xhr || this.config.request.restful) {
    return res.json({
      type: 'error',
      code: 'INTERNAL_ERROR',
