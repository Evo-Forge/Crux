...
function checkViews() {
  ...
  try {
    ...
    _.forEach(errorFiles, function(fullPath) {
      ...
      if(!isNaN(code)) {
        var viewPath = fullPath.replace(Component.appPath(self.config.path.views.base), '');
        errorViews[errCode] = viewPath;
      }
    });
  } catch(e) {}
};
...
function loadRoutes() {
  try {
    var list = Krux.util.readDirectory(Component.appPath(this.config.path.routes), 'js'),
      self = this;
    if(list.length === 0) throw new Error('No routes');
  } catch(e) {
    log.warn('Krux.server: component has no routes.');
    return;
  }
...
Server.prototype.createHttpRoute = function CreateHttpRoute(routePath) {
  ...
  var httpRouteObj = new HttpRoute(this, namespace);
  return httpRouteObj;
};
...
* */
Server.prototype.__render = function GetRenderCallback() {
  // TODO:
...
Server.Route = Server.prototype.Route = HttpRoute;
Server.Request = Server.prototype.Request = HttpRequest;
Server.Validations = Server.prototype.Validations = HttpValidations;
