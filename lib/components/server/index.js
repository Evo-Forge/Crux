...
var Server = function KruxServerComponent() {
...
function checkViews() {
  var self = this;
  try {
    var errPath = Component.appPath(this.config.path.views.base, this.config.path.views.errors);
    var errorFiles = Krux.util.readDirectory(errPath, this.config.path.views.extension, null, true);
    _.forEach(errorFiles, function(fullPath) {
      var errCode = fullPath.replace(errPath, '').replace('.' + self.config.path.views.extension, '');
      var code = parseInt(errCode);
      if(!isNaN(code)) {
        errorViews[errCode] = fullPath.replace(Component.appPath(self.config.path.views.base), '');
      }
    });
  } catch(e) {}
}
...
function loadRoutes() {
  try {
    var list = Krux.util.readDirectory(Component.appPath(this.config.path.routes), 'js'),
      self = this;
    if(list.length === 0) {
      return log.warn('Krux.server: component has no routes.');
    }
  } catch(e) {
    return;
  }
  ...
  list.forEach(function(routePath) {
    ...
    if(!_.isFunction(routeObj)) {
      log.warn('Krux.server: Route %s does not export a function.', httpRouteObj.__rootPath);
      return;
    }
  ...
  log.trace('Krux.server: %s route(s) loaded.', routeCount);
}
...
function createDocs() {
  try {
    var hasDocFolder = fs.existsSync(Component.appPath(this.config.path.docs.folder)),
      hasTemplate = fs.existsSync(Component.appPath(this.config.path.docs.template));
    if(!hasDocFolder || !hasTemplate) throw new Error("No docs");
  } catch(e) {
    // No documentation support.
    return;
  }
  var docTwig = fs.readFileSync(Component.appPath(this.config.path.docs.template), { encoding: 'utf8' }).toString();
  ...
  try {
    fs.writeFileSync(Component.appPath(this.config.path.docs.folder, 'doc.html'), html, { flag: 'w', encoding: 'utf8' });
    log.trace('KruxServer: Documented %s routes in ' + this.config.path.docs.folder, routeCount);
  } catch(e) {}
}
...
Server.prototype.__render = function GetRenderCallback(path, vars, cb) {
  return swig.renderFile(path, vars, function(err, html) {
    cb(err, html);
  });
};
...
Server.prototype.render = function RenderContent(viewPath, _locals, cb) {
  var fullPath = path.normalize(Component.appPath(this.config.path.views.base) + '/' + viewPath);
  if(fullPath.indexOf(this.config.path.views.extension) === -1) {
    var _ext = (this.config.path.views.extension.charAt(0) === '.' ? '' : '.') + this.config.path.views.extension;
    fullPath += _ext;
  }
...
Server.prototype.configureDefaults = function Configure() {
  ...
  this.app.engine(this.config.render, this.__render);
  this.app.set('view engine', this.config.render);
  this.app.set('views', Component.appPath(this.config.path.views.base));
  this.app.set('view cache', false);
  swig.setDefaults({
    cache: this.config.path.views.cache,
    root: Component.appPath(this.config.path.views.base)
  });
  ...
  if(this.config.request.cors) {
    this.app.use(function CORSRequest(req, res, next) {
      ...
      res.header('Access-Controll-Allow-Methods', 'GET POST DELETE PUT OPTIONS');
      return next();
    });
  }
...
Server.prototype.bind = function Bind(callback) {
  ...
  this.http.on('error', function(err) {
    ...
    log.error('Krux.server: encountered an error.');
    log.debug(err);
  });
  this.http.listen(this.config.port, this.config.ip, function() {
    if(cbCalled) return;
    log.info('KruxServer started on %s:%s', self.config.ip, self.config.port);
    cbCalled = true;
    callback();
  });
};
...
Server.prototype.serverError = function ServerError(err, req, res, next) {
  ...
  log.warn('KruxServer Encountered an error in [%s %s]' , req.method, req.___routeName);
