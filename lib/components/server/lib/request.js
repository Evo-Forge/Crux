...
Request.prototype.isAjax = function IsAjaxRequest() {
  if(SERVER.config.request.restful) return true;
  try {
  ...
  if(this.req.method === 'PUT') return true;
...
Request.prototype.render = function RenderTemplate(name, options) {
  ...
  this.__clearClose();
  var self = this;
  SERVER.render(name, _opt, function(err, html) {
    _opt = null;
    if(err) {
      return SERVER.serverError(err, self.req, self.res, null);
    }
    self.send(html);
  });
};
...
Request.prototype.error = function AjaxError(_code, message, data) {
  ...
  if(_.isObject(_code)) {
    ...
    if(typeof _code.statusCode === 'number') {
      statusCode = _code.statusCode;
    }
  } else {
    code = _code;
  }
  ...
  if(_.isString(message)) {
    if(process.env.NODE_ENV === 'production' && message.toLowerCase().indexOf('crux.') !== -1) {
      message = 'An error occurred.';
    }
  ...
  if(statusCode && !this.res._headerSent) {
...
* */
Request.prototype.redirect = function Redirect(path, _status) {
  if(typeof _status === 'number') {
    this.res.redirect(_status, path);
  } else {
    try {
...
/**
* Helper method that returns the lowercase hostname of the request.
 * @memberof crux.Server.Request
 * @function redirect
 * @instance
* */
Request.prototype.getHost = function GetHostname() {
  return (this.req.host || '').toLowerCase();
};
