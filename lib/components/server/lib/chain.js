...
* */
var chain = function CruxRouteChain(routeName, routePath, routeMethod, SERVER) {
  this.name = routeName;
  if(typeof routePath === 'string') this.path = routePath;
  if(typeof routeMethod === 'string') this.method = routeMethod;
  this.thenChain = [];
  if(SERVER.config.request.mocking) {
...
chain.prototype.mock = function MockRequest(mockCallback) {
  if(typeof this.mockCallback === 'undefined') {
    return this;  // mocking is disabled, we ignore the call.
  }
