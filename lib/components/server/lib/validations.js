 ...
 * Performs string validation. Note that an emptry string is not valid.
 * @memberof crux.Server.Validations
 * @function STRING_FULL
 * @static
 * @param {String} name - the name of the property inside the source object
 * @param {Object} source - the source object to look for the array in
 * */
validation.STRING_FULL = function ValidateString(name, source) {
  if(typeof source[name] !== 'string') return false;
  var trimmed = source[name].trim();
  if(trimmed.length === 0) return false;
  source[name] = trimmed;
...
validation.ENUM = function BuildEnumValidation(arra) {
  ...
  function ValidateEnum(name, source) {
    if(!validation.STRING(name, source)) {
      if(_default) {
        source[name] = (typeof _default === 'object' && _default ? JSON.parse(JSON.stringify(_default)) : _default);
        return true;
      }
      return false;
    }
  ...
  ValidateEnum.invalid = function onInvalidData(code, message, _statusCode) {
    var err = new Error();
    ...
    if(typeof _statusCode === 'number') {
      err.statusCode = _statusCode;
    }
...
/**
 * URL Validations
 * VALID urls contain as follows:
 * [http|https]://[domain|lowercase()]/[path][?query]
 * */
validation.URL = function ValidateURL(name, source) {
  if(!validation.STRING(name, source)) return false;
  var full = source[name],
    urlObj;
  try {
    urlObj = url.parse(full);
  } catch(e) {
    return false;
  }
  if(urlObj.protocol !== 'http:' && urlObj.protocol !== 'https:') return false;
  if(!urlObj.host) return false;
  urlObj.host = urlObj.host.toLowerCase();
  var tmpHost = urlObj.host.split('.'),
    isLocal = false;
  ...
  var d = {
    ...
    path: urlObj.path,
    query: urlObj.query,
    port: urlObj.port ? urlObj.port : (urlObj.protocol === 'http:' ? 80 : 443)
  };
  source[name] = d;
  return true;
};
...
// We now bind the "default" call to each validation type, and their type
  _.forEach(validation, function(validateFunc, key) {
    ...
    function onInvalidData(code, message, _statusCode) {
      var err = new Error();
      if(typeof message === 'undefined') {
        err.code = 'INVALID_DATA';
        err.message = code;
