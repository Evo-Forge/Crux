...
function KruxServerRoute() {
  ...
  HttpRoute.prototype.type = Validations;
    ...
    var securityCallback = function SecurityChainCallback() {
      ...
      for(var i=0; i < securityParams.url.length; i++) {
        ...
        if(!p.validate(p.name, this.req.params)) {
          return this.fail('URL_PARAMETERS', 'Invalid or missing URL parameter', (ENV === 'dev' ? [p.name] : null));
        }
      }
      ...
      if(missingQs.length !== 0) {
        return this.fail('QUERY_PARAMETERS', "Invalid or missing querystring parameters", (ENV === 'dev' ? missingQs : null));
      }
      /* Validate post data */
      var missingPost = [];
      ...
      if(missingPost.length !== 0) {
        return this.fail('BODY_PARAMETERS', "Invalid or missing body parameters", (ENV === 'dev' ? missingPost : null));
      }
      ...
      if(missingHeader.length !== 0) {
        return this.fail('HEADER_PARAMETERS', 'Invalid or missing header parameters', (ENV === 'dev' ? missingHeader : null));
      }
  ...
  * */
  var bindEndpoint = function BindHttpEndpoint(endpoint, req, res, httpPath, _routeCallbackName) {
    ...
    var routeContext = this.createContext(req, res, httpPath);
    ...
    if(missingQs.length !== 0) {
      return routeContext.error('QUERY_PARAMETERS', "Invalid or missing querystring parameters", (ENV === 'dev' ? missingQs : null));
    }
    ...
    if(missingPost.length !== 0) {
      return routeContext.error('BODY_PARAMETERS', "Invalid or missing body parameters", (ENV === 'dev' ? missingPost : null));
    }
...
module.exports = function(a,b,c,d,e,f,g) {
  var RouteModel = KruxServerRoute();
  return new RouteModel(a,b,c,d,e,f,g);
};