...
function CruxServerRoute() {
  ...
  HttpRoute.prototype.bind = function Bind() {
    ...
    _.forEach(this.__endpoints, function(endpoint) {
      ...
      APP[endpoint.method](httpPath, function(req, res) {
        self.__bindEndpoint.call(self, endpoint, req, res, httpPath);
      });
      ...
      * */
      ...
      APP[endpoint.method](httpMockPath, function(req, res) {
        ...
        self.__bindEndpoint.call(self, endpoint, req, res, httpMockPath, "mockCallback");
      });
    });
    isBinded = true;
  };
  ...
  * */
  HttpRoute.prototype.createContext = function CreateRouteContext(req, res, path) {
    ...
    /*
     * Performs a proxy on the current route callback, passing the handler to the given
     * endpoint id. NOTE that a proxy request will create a separate context object,
     * basically simulating a new HTTP request.
     * */
    if(typeof HttpRequest.prototype.proxy !== 'function') {
      HttpRequest.prototype.proxy = function DoProxy(endpointId) {
        try {
          var namespace = endpointId.split(':')[0];
        } catch(e) {
          log.warn('Invalid endpoint name on request proxy(): %s', endpointId);
          return this.error(e);
        }
        var ns = SERVER.routes[namespace],
          endpointObj;
        for(var key in ns.__endpoints) {
          if(ns.__endpoints[key].id === endpointId) {
            endpointObj = ns.__endpoints[key];
            break;
          }
        }
        if(!endpointObj) {
          log.warn('Invalid endpoint name on request proxy(): %s', endpointId);
          return this.error(new Error('Invalid endpoint ID'));
        }
        self.__bindEndpoint(endpointObj, this.req, this.res, endpointObj.path);
      };
    }
    return requestObj;
  };
