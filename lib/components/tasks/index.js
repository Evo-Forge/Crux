...
tasks.prototype.get = function GetTask(name) {
  if(typeof REGISTERED_TASKS[name] === 'undefined') return null;
...
tasks.prototype.attachTo = function AttachTasks() {
  ...
  process.nextTick(function() {
    function runTask(name) {
      var tObj = self.get(name);
      if(!tObj) return null;
      return tObj.run(true);
    }
    self.registry().attachTo('server service tasks', 'runTask', runTask);
    self.registry().attachTo('server service tasks', 'getTask', self.get.bind(self));
  });
};
