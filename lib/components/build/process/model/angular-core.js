...
core.prototype.watch = function WatchAngularCore(done) {
  ...
  this.__watch(this.config.path, this.config.extension, onChange, function onWatchEnd(err) {
    if(self.config.include.length === 0 || err) return done(err);
    // we have to do the same for the included files.
    var incs = [];
    _.forEach(self.config.include, function(includePath) {
      incs.push(function(onWatchDone) {
        var watchPath = path.normalize(__rootdir + '/' + includePath);
        self.__watch(watchPath, self.config.extension, onChange, onWatchDone);
      });
    });
    async.series(incs, done);
  });
};
