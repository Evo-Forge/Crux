...
var template = function KruxBuildAngularTemplates(config) {
...
template.default = {
  ...
  output: 'app/public/build/$module.templates.js',  // The js output file, where $name is the module name
  module: 'app',
  ...
  for(var viewPath in this.CACHE) {
    ...
    if(typeof this.config.compile === 'function') {// Custom compilation javascript
      ...
      if(typeof viewJs !== 'string' || viewJs == '') {
        var err = new Error('Krux.build.angular:templates config.compile() does not return a string.');
        this.emit('error', err);
        return done && done(err);
      }
    } else {
      viewJs = 'angular.module("'+ this.config.module +'").run(["$templateCache", function(t) {';
