...
var process = function KruxBuildSass() {
...
process.prototype.init = function Initialize() {
  if(this.paths.length > 1) {
    throw new Error('Krux.build.sass: This process only supports one source directory.');
  }
...
process.prototype.run = function RunSass(done) {
  ...
  if(this.config.includes.length) {
    ...
    for(var i=0; i < this.config.includes.length; i++) {
      var inc = path.normalize(process.cwd() + '/' + this.config.includes[i]);
