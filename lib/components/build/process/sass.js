...
/**
 * This is the SASS build compiler. It will compile any .scss files configured. It uses <b>node-sass@3.4.2</b> npm package  to do so.
 *
 * @memberof crux.Build
 * @class Sass
 ...
 * @param {Array} [config.includes] - additional file paths to be included while compiling the sass sources
...
process.prototype.packages = function Dependencies() {
  return ['node-sass@3.4.2', 'node-watch@0.3.4'];
};
...
process.prototype.run = function RunSass(done) {
  ...
  sass.render(_opt, function(err, result) {
    if(err) {
    ...
    var css = (typeof result === 'string' ? result : result.css);
    self.output(css, done);
  });
};
