...
process.prototype.packages = function Dependencies() {
  var req = ['browserify@11.0.1', 'aliasify@1.7.2', 'node-watch@0.3.4'];
  if(this.config.uglify) {
    req.push('uglify-js@2.4.x');
  }
  return req;
};
...
process.default = {
  ...
  aliasify : true,
  uglify: false,
  map: false,         // should we create file.js.map. This should be used with uglify=true
...
process.prototype.save = function SaveAppFile(jsContent, onSave) {
  ...
  if(this.config.uglify) {
    var result = uglify.minify(jsContent, {
      fromString: true,
      outSourceMap: (self.config.map ? Kutil.getFileName(this.config.output) + '.map' : false)
    });
...
process.prototype.build = function OnBuild() {
  ...
  this.run(function(e) {
    if(e) {
      e.message = 'Crux.build.commonjs: ' + e.message;
      self.emit('error', e);
    }
  });
};
