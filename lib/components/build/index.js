...
*   - Custom Krux frontend framework (will soon be deprecated)
...
/*
* The function will programatically enable a core component by setting the given configuration file
...
Build.prototype.process = function EnableCoreProcess(name, config) {
  if(typeof name !== 'string' || !name) {
    throw new Error('Krux.build: process() received invalid name');
  }
  if(!hasProcess(name, 'core')) {
    throw new Error('Krux.build: process() failed, "'+name+'" is not a core process.');
  }
...
Build.prototype.custom = function AddCustomProcess(name, classDefinition, _config) {
  if(typeof name !== 'string' || !name) {
    throw new Error('Krux.build: custom() received invalid name');
  }
  if(hasProcess(name, 'core')) {
    throw new Error('Krux.build: custom() failed, "'+name+'" is a core process and cannot be defined as a custom one.');
  }
  if(hasProcess(name, 'custom')) {
    throw new Error('Krux.build: custom() failed, "'+name+'" was previously added.');
  }
  if(classDefinition instanceof Interface) {
    throw new Error('Krux.build: custom() failed, "'+name+'" must be the process class definition, not class instance.');
  }
  if(classDefinition.super_ !== Interface) {
    throw new Error('Krux.build: custom() failed, "'+name+'" must extend Build.Interface');
  }
  ...
  if(typeof _config === 'object' && _config !== null) {
    this.config.process[name] = Krux.util.extend(true, this.config.process[name] || {}, _config);
  }
  return this;
};
...
Build.prototype.init = function Initialize() {
  ...
  _.forEach(this.config.process, function(processConfig, processName) {
    if(!hasProcess(processName)) {
      log.warn('Krux.build: "%s" is not loaded or does not exist. Skipping', processName);
      return;
    }
    var procDef = getProcess(processName),
      procConfig = Krux.util.extend(true, procDef.default || {}, processConfig);
...
Build.prototype.set = function SetConfigProperty(key, value) {
  if(typeof key !== 'string' || key === '') {
    throw new Error('Krux.build: invalid configuration key via set()');
  }
  if(key === 'process') {
    throw new Error('Krux.build: configuration key "process" is restricted via set()');
  }
...
* */
Build.prototype.run = function RunBuild(done) {
  var self = this;
  installRequirements.call(this, function(err) {
    ...
    _.forEach(self.loaded, function(procObj) {
      ...
      _runs.push(function(onRunComplete) {
        ...
        procObj.run(onRunComplete);
      });
    });
    async.waterfall(_runs, function onRunComplete(err) {
      if(err) return done(err);
      // After we've run every process, we will start watching them.
      if(!self.config.autoWatch) {
        return done();
      }
      var _watches = [];
      _.forEach(self.loaded, function(procObj) {
        if(procObj.config.autoWatch === false) return;
        if(procObj.paths.length === 0) return;
        _watches.push(function(watchDone) {
          procObj.watch(watchDone);
        });
      });
      async.waterfall(_watches, done);
    });
  });
};
...
function bindProcess(procObj) {
  var self = this;
  procObj.on('change', function onChange(path) {
    self.emit('change', procObj.name, path);
  }).on('build', function onBuild() {
    if(self.config.debug) {
      log.trace('Krux.build.' + procObj.name + " compiled.");
    }
    ...
    log.error('Krux.build: Process %s encountered an error', procObj.name);
    log.debug(err);
  });
};
...
function installRequirements(done) {
  ...
  _.forEach(REQUIRED_INSTALLS, function(npmName) {
    _calls.push(function(done) {
      return Krux.util.install.npm(npmName, done);
    });
  });
...
var list = Krux.util.readDirectory(__dirname + '/process', 'js', null, true);
_.forEach(list, function(coreDep) {
  var name = Krux.util.getFileName(coreDep, false);
  name = Krux.util.capitalize(name);
