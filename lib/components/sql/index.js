var async = require('async'),
  ...
  modelCount = 0;
/*
 * Krux Database Component (Sequelize wrapper)
 * */
...
var DbService = function KruxSqlComponent(_config) {
...
DbService.prototype.Register = function CreateModel(name, tableName) {
  return new Model(name, tableName);
};
...
DbService.prototype.attach = function AttachDatabase() {
  ...
  this.registry().attachTo('server service', 'model', getModel);
};
...
DbService.prototype.run = function Initialize(done) {
  ...
  this.Seq.authenticate().then(function() {
    ...
    log.fatal('Krux.sql: failed to connect to server. Please check db configuration.');
    log.debug(err);
    return done(err);
  });
};
 ...
 * */
DbService.prototype.Model = function GetModel(name) {
  if(typeof MODELS[name] === 'undefined') {
    throw new Error("Krux.sql: Invalid database model definition for: " + name);
  }
  return MODELS[name];
};
...
DbService.prototype.customTypes = function SetCustomTypes() {
  ...
  Sequelize.JSON.toString = function toString() { return 'VARCHAR(500)'; }
...
DbService.prototype.synchronize = function Synchronize(onComplete) {
  ...
  if(this.config.sync === false) {
    isSyncFinished = true;
    log.info('Krux.sql: connection established.');
    return onComplete();
  }
  // If we're performing a database reset, we need to do the following steps.
  if(self.config.setup) {
    log.info('Krux.sql: Setting up database structure');
    var opt = {
      force: true
    };
    ...
    self.Seq.query('SET FOREIGN_KEY_CHECKS = 0')
      ...
      .then(function() {
        ...
        try {
          var sqlFiles = Krux.util.readDirectory(Component.appPath(self.config.path.sql), 'sql');
        } catch(e) {
          isSyncFinished = true;
          log.debug('Krux.sql: Database setup complete');
          return onComplete();
        }
        var _calls = [];
        ...
        async.series(_calls, function() {
          log.trace('Krux.sql: SQL synchronization complete.');
          isSyncFinished = true;
          return onComplete();
        });
      }, function(err) {
        log.warn('Krux.sql: SQL file queries encountered an error.');
        log.error(err);
        return onComplete();
      });
    return;
  ...
  this.Seq.sync().success(function() {
    log.info('Krux.sql: database synchronization complete.');
    isSyncFinished = true;
    onComplete();
  }).error(function(err) {
    log.warn('Krux.sql: Database synchronization encountered an error.');
    log.error(err);
    isSyncFinished = true;
    return onComplete();
  });
...
DbService.prototype.runSql = function RunSqlFromFile(_path, _done) {
  ...
  try {
    ...
    log.warn('Krux.sql: failed to run SQL from file: %s, cannot read from it.', fileName);
    log.debug(e);
    return _done();
  }
  ...
  _.forEach(setupSql, function(sqlStatement) {
    ...
    asyncCalls.push(function(done) {
      self.Seq.query(sqlStatement).then(function() {
        log.trace('Krux.sql: run SQL qyery from file %s', fileName);
        done();
      }, function(err) {
        log.warn('Krux.sql: Failed to run SQL query from file: %s', fileName);
        log.debug(err);
        done();
      });
    });
  });
  async.waterfall(asyncCalls, _done);
};
...
function loadModels() {
  var MODEL_LOCATION = Component.appPath(this.config.path.models),
    self = this;
  try {
    var list = Krux.util.readDirectory(MODEL_LOCATION, 'js');
  } catch(e) {
    // Component has no models.
    log.warn('Krux.sql: no model definitions found in %s', this.config.path.models);
    return;
  };
  ...
  list.forEach(function(modelPath) {
    ...
    var modelName = Krux.util.capitalize(tableName, true);
    var modelObj = self.Register(modelName, tableName);
    var modelFunc = require(modelPath);
    if(typeof modelFunc !== 'function') {
      log.warn('Krux.sql: failed to load model definition "%s", not a function', modelName);
      return;
    }
    modelFunc(modelObj, Sequelize);
    if(!modelObj.hasFields) {
      log.warn('Krux.sql: Model definition "%s" has no fields. Skipping', modelName);
      return;
    }
    ...
    MODELS[modelObj.name] = self.Seq.define(modelObj.name, modelObj.fields, _opt);
    modelObj.bindMethods(MODELS[modelObj.name]);
    if(modelObj.hasRelationships) {
      RELATIONSHIPS.push(modelObj);
    }
  });
  RELATIONSHIPS.forEach(function(modelObj) {
    var sourceModel = MODELS[modelObj.name];
    /* Has many */
    _.forEach(modelObj.__hasMany, function(depOpt, depName) {
      ...
      try {
        ...
        log.fatal('Failed to do hasMany on %s-%s',depName, modelObj.name);
        log.debug(e);
      }
    });
    /* Has one */
    _.forEach(modelObj.__hasOne, function(depOpt, depName) {
      ...
      try {
        ...
        log.fatal('Failed to do hasOne on %s-%s', depName, modelObj.name);
        log.debug(e);
      }
    });
    /* Belongs to */
    _.forEach(modelObj.__belongsTo, function(depOpt, depName) {
      ...
      try {
        ...
        log.fatal('Failed to do belongsTo on %s-%s', depName, modelObj.name);
        log.debug(e);
      }
    });
  });
    ...
    modelObj.mock = function MockJsonModel(data) {
      ...
      for(var attr in modelObj.rawAttributes) {
        ...
        if(aName.indexOf("VARCHAR") !== -1 && (typeof rawAttr.type._parseJson === 'undefined')) {
          mockData[attr] = Krux.util.uniqueId(5);
        }
...
DbService.prototype.Model = Model;
