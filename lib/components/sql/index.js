...
DbService.prototype.init = function InitializeDB() {
  ...
  if(this.config.setup) {
    ...
    if(typeof this.config.connections === 'number') {
      opt.pool.maxConnections = this.config.connections;
    }
  }
  ...
  opt['logging'] = (!this.config.debug ? false : function(msg) {
    if(!authCalled) {
      authCalled = true;
      return;
    }
...
DbService.prototype.attach = function AttachDatabase() {
  process.nextTick(function() {
    ...
    function startTransaction(_mode, _options) {
      ...
      return Crux.promise(function(resolve, reject) {
        ...
        self.Seq.transaction(options, function(t) {
          ...
          log.warn('Crux.transaction() actual transaction encountered an error.');
          log.debug(e);
    ...
    function startTransaction2(_mode, _options) {
      var cb = (typeof _mode === 'function' ? _mode : _options);
      return crux.promise(function(resolve, reject) {
        var _error = null,
          _result = null,
        ...
        var transactionPromise = self.Seq.transaction(_options, function(t) {
          var _rollback = t.rollback,
            _commit = t.commit,
            isCommited = false,
            isRolledBack = false;
          t.rollback = function CruxTransactionRollback(error) {
            if(isCommited || isRolledBack) {
              log.error('Crux.transaction(): is already ' + (isCommited ? 'committed' : 'rolled back') + '. Cannot perform multiple commits.');
              return;
            }
            isRolledBack = true;
            _error = error;
            return _rollback.call(t);
          };
          t.commit = function CruxTransactionCommit(result) {
            if(isCommited || isRolledBack) {
              log.error('Crux.transaction(): is already ' + (isCommited ? 'committed' : 'rolled back') + '. Cannot perform multiple commits.');
              return;
            }
            isCommited = true;
            _result = result;
            return _commit.call(t);
          };

          try {
            var cbPromise = cb({
              transaction: t,
              commit: t.commit.bind(t),
              rollback: t.rollback.bind(t)
            });
          } catch(e) {
            log.error('Crux.transaction(): callback function encountered an error.');
            log.error(e);
          ...
          if(typeof cbPromise === 'object' && cbPromise && typeof cbPromise.catch === 'function') {
            cbPromise.catch(function(err) {
              t.rollback(err);
            });
          }
          return cbPromise;
        });
        return transactionPromise.then(function() {
          if(completed) return;
          completed = true;
          if(_error) {
            return reject(_error);
