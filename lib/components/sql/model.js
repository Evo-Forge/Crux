...
DbModel.prototype.instanceMethods = function BindInstanceMethods() {
  ...
  this.method('isNew', function IsNewRecord() {
...
DbModel.prototype.bindMethods = function BindModelMethods(modelObj) {
  ...
  if(typeof modelObj['NOT_FOUND'] === 'undefined') {
    modelObj['NOT_FOUND'] = {
      code: 'NOT_FOUND',
      message: 'The requested ' + this.name + ' was not found.'
    }
  }
  return this;
};
 ...
 * */
DbModel.prototype.index = function AddIndex(name, fields, _opt) {
...
/*
 * Marks the given fields as being cryptkeys
 * */
DbModel.prototype.cryptkey = function MarkAsCryptKey(fields) {
  if(_.isString(fields)) {
    fields = [fields];
  }
  if(!_.isArray(fields)) {
    throw new Error('Invalid crypt-key fields: ' + fields + ' for ' + this.name);
  }
  // TODO
  //console.log("CRYPT", fields);
};
  ...
  if (typeof type._parseJson === 'boolean' && type._parseJson === true) {
    ...
    if (typeof options['defaultValue'] === 'object' && options['defaultValue'] !== null) {
      try {
        ...
        log.fatal('Invalid JSON default value for field %s on model %s', name, this.name);
        log.error(e);
        return this;
      }
    } else {
...
DbModel.prototype.belongsTo = function BelongsTo(name, options) {
  if (typeof this.__belongsTo[name] !== 'undefined') {
    throw new Error("Cannot re-declare belongsTo " + name + ' for model ' + this.name);
  }
...
DbModel.prototype.hasMany = function HasMany(name, options) {
  if(typeof this.__belongsTo[name] !== 'undefined') {
    throw new Error("Cannot re-declare hasMany " + name + ' for model ' + this.name);
  }
  if(typeof options === 'undefined') options = {};
  if(typeof options['foreignKey'] === 'undefined') {
    options['foreignKey'] = this.name + "_id";
  }
  if(typeof options['onDelete'] === 'undefined') {
    options['onDelete'] = 'cascade'
  }
...
DbModel.prototype.hasOne = function HasOne(name, options) {
  if(typeof this.__belongsTo[name] !== 'undefined') {
    throw new Error("Cannot re-declare hasOne " + name + ' for model ' + this.name);
  }
  if(typeof options === 'undefined') options = {};
  if(typeof options['foreignKey'] === 'undefined') {
    options['foreignKey'] = this.name + "_id";
