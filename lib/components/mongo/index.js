...
var Mongo = function KruxMongoComponent() {
...
function loadModels() {
  ...
  try {
    ...
    log.warn('Krux.mongo: no schema definitions found in %s', this.config.schemas);
    return;
  };
  ...
  list.forEach(function(modelPath) {
    ...
    var schemaName = Krux.util.capitalize(colName, true);
    var modelObj = self.Register(schemaName, colName);
    var modelFunc = require(modelPath);
    if(typeof modelFunc !== 'function') {
      log.warn('Krux.mongo: failed to load schema %s, not a function', schemaName);
      return;
    }
    var modelObj = self.Register(schemaName, colName);
    modelFunc(modelObj, self.getTypes());
    if(!modelObj.hasFields) {
      log.warn('Krux.mongo: schema definition "%s" has no fields. Skipping', colName);
      return;
    }
    if(typeof self.schemas[modelObj.__name] !== 'undefined') {
      throw new Error('Krux.mongo: Schema ' + modelObj.__name + ' previously defined.');
    }
...
Mongo.prototype.run = function InitializeMongo(done) {
  ...
  function onConnect(err, res) {
    ...
    if(err) {
      if(err.code === 18) {
        log.fatal('Krux.mongo: Failed to authenticate to MongoDb. Please check configuration.');
      } else {
        log.fatal('Krux.mongo: Failed to connect to Mongodb. Please check configuration.');
        log.debug(err);
      }
      return done(err);
    }
  ...
  this.mongo.connection.on('connected', function() {
    log.info('Krux.mongo: connection established.');
    if(isCalled) return;
    isConnected = true;
    isCalled = true;
    done();
    ...
    log.error('Krux.mongo: Connection terminated.');
  }).on('reconnected', function() {
    log.info('Krux.mongo: Connection re-established.');
  }).on('error', function(e) {
    log.error('Krux.mongo: Connection encountered an error');
    log.debug(e);
  });
};
...
Mongo.prototype.Schema = function GetSchema(name) {
  if(typeof this.schemas[name] === 'undefined') {
    throw new Error('Krux.mongo: Invalid mongo schema for: ' + name);
  }
