...
Mongo.prototype.attach = function AttachDatabase() {
  ...
  process.nextTick(function() {
    ...
    * */
    ...
    /*
    * Will return collection count using distinct field 
    * provided.
    * */
    function collectionDistinct(name, qry, field) {
      return Crux.promise(function(resolve, reject) {
        var col = self.mongo.connection.db.collection(name);
        col.distinct(field, qry || {}, function(err, items){
          if(err) {
            return reject(err)
          }
          resolve(items);
    ...
    /*
    * Simple collection aggregate for mongo
    * */
    function collectionAggregate(name, qry, opts) {
      return Crux.promise(function(resolve, reject) {
        var col = self.mongo.connection.db.collection(name);
        col.aggregate(qry, opts || {}, function(err, count) {
          if(err) {
            return reject(err);
          }
          resolve(count);
    ...
    self.registry().attachTo('server service tasks io', 'collectionDistinct', collectionDistinct);
    self.registry().attachTo('server service tasks io', 'collectionAggregate', collectionAggregate);
